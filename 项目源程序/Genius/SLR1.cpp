...
*  @note v3.0      梁海峰      2024-5-29   完善代码, 使其minic语言的文法分析过程表与抽象语法树生成
*****************************************************************************
*/
...
// 函数名称: 父子关系调整函数
// 函数功能: 根据修改模式调整父结点的名字/值/子节点
// 函数参数: treeNode* 子节点, treeNode* 父节点, mode 修改模式, 为三位二进制数转成的十进制数
// 函数返回值:int 是否保留当前子节点的标志
int SLR1::shiftAst(treeNode* child, treeNode* parent, int mode)
{
    if (mode <= 0)
    {
        return 1; // 保留结点默认(-1)不删除子节点
    }
    cout << endl << endl << parent->name << "|" << parent->num << "仅有孩子" << child->name << "|" << child->num << ", 需要坍塌" << endl;
    string cname = child->name;

    int node = mode % 2; // 是否保留当前结点的标志, 1则保留
    int value = (mode / 2) % 2;// 是否用子节点值替代父节点的标志, 1则替代
    int name = (mode / 4) % 2; // 是否用子节点的名字替代父节点的标志, 1则替代
    ...
    {
        cout << "完全取代" << endl;
        parent->astChild = child->astChild;
        return 1;// 完全取代就不删除子节点
    }
    return node;
}
...
void SLR1::geneAST(treeNode* root, treeNode* astParent)
{
    ...
    {  
        ...
        cout <<endl<<endl<< "当前叶子为" << root->name<<"|"<<root->num << ", 当前父亲为" << astParent->str()<<"|" << astParent->num << endl;

        cout << "到达可叶子结点" << root->name <<endl;
        string op = ">@<@>=@<=@!=@==@<>@+=@-=@*=@/=@%=@++@--@&&@||@";
        if (op.find(name) == string::npos) // 根据声明的特殊关系处理非运算符的叶子结点
        {
            string t = pname + '~' + name;
            if (isAst[t] > 0) // 需要特殊处理的叶子结点
            {
                ...
                {
                    cout << name << "特殊保留模式, 将作为孩子插入到" << astParent->name << "结点" << endl;
        ...
        else // 叶子结点是运算符, 会顶替父节点, 具体实现就是不生成结点, 但是会顶替父节点的值
        {  
        ...
        cout << name << "普通保留模式, 将作为孩子插入到" << astParent->name << "结点" << endl;
        int size = astParent->astChild.size();
        astParent->astChild[size] = root;
        return;
    }
    ...
    int testnum1 = root->num;

    if ( root->astChild.size() == 1) // 如果当前结点前序遍历完后, 孩子结点只有一个, 为了美观, 直接将其与父节点视情况合并部分
    {
        treeNode* child = root->astChild.begin()->second;
        if (shiftAst(child, root, isAst[root->name]) == 0) // 父节点保留标志位(第三位)为0说明不保存该非保留结点
            root->astChild.clear();
        //if (isAst[child->name] == 0) // 保留结点不去删除, 仅处理不保留结点
        //{   
        //    if (shiftAst(child, root, isAst[root->name]) == 0) // 父节点保留标志位(第三位)为0说明不保存该非保留结点
        //        root->astChild.clear();
        //}
